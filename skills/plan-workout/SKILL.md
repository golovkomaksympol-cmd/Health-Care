...
5. **Coverage & patches:** check the week — fill under-trained patterns + **the person's own weak links as recorded in their `Health profile` / `Practices`**
   - **Check out-of-gym load before calling a pattern uncovered.** The gym log is not the whole training load. Running, cycling, hiking, dance, manual work and even **footwear** load real tissues that never appear in a session log — e.g. low-drop / minimalist shoes shift load from knee to calf and Achilles, so a runner in flat shoes accumulates thousands of loading cycles per run against the few dozen in a calf set. Reading "no calf work in the logs" as "calf is uncovered" prescribes a **second** stimulus onto already-loaded, possibly symptomatic tissue. Ask what the person does outside the gym before filling a gap that isn't there.
   - **Rehab protocols are treatments, not supplements.** An item flagged `Rehab` (Alfredson, tendon loading, TKE) is indicated by a **diagnosis**, not by absence from the log. State the indication; if it isn't present, don't prescribe it. Where capacity is genuinely in question, prescribe a one-off **capacity test** instead of guessing from the log — and read **side-to-side asymmetry**, which is more informative than the absolute number.
   - **Pattern coverage ≠ staple coverage.** A pattern can look "covered" by its big compound while a habitual accessory silently disappears for months — e.g. hinge covered by the RDL every week while the 45° Back Extension, once done nearly every session, hasn't appeared since spring. Scan further back than the last 5 sessions for **exercises the person used to do regularly and no longer does**; if one has dropped out, either reinstate it or say out loud that you're dropping it and why. Losing it costs a real capacity (there: extensor endurance, which `Core Stability.md` tests separately from the lift). (typical classes: a rehab-flagged tendon, a lagging single-joint muscle, a side-to-side asymmetry, timing/activation faults). Don't assume weak links that aren't documented.
...
- **No parenthetical or "optional" exercises.** Every exercise that appears in the output — including a time-permitting extra — gets the full treatment: sets × reps, load, and its catalog link. If it isn't worth prescribing properly, leave it out. An aside like *"(optional: Biceps Curl / Lateral Raises)"* is a bug: it skips the format, and its links never get looked up.
- **Name the variant, and why — for every exercise that has one.** Check the prescribed exercise against **Variant selection** at the end of `Exercise Catalog.md`; if it's listed there, the setup decides which muscle actually works, so state it in a short clause: *foot position, torso lean, grip, bench angle, knee angle, swing height.* Format: `Machine Hip Thrust — 3×12, 130 kg *(shin vertical at lockout → glutes: quads got two front-squat days, hamstrings are loaded by today's RDL)*`. Pick the variant by two rules: (1) it serves the pattern **this session is for**, (2) it does **not** double a muscle already loaded elsewhere today. If the exercise isn't in that table, don't invent a variant.
