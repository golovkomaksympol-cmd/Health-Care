...
| Machine | Good for | Where it bites | Cue / when to prefer it | Verdict | Better alternative |
|---|---|---|---|---|---|
| **Leg Curl — lying** | hamstring isolation | prone with hips flat → hamstring shortens at both joints (active insufficiency), hips lift, **lumbar extension under load** | angled pad (hips ~30°) fixes most of it; otherwise choose seated | avoid if a seated one exists | **Seated Leg Curl**; if none — Single-leg RDL or Nordic curl (hamstring at long length) |
...
| **Hip Abduction / Adduction (seated)** | targeted glute-med (abd) and adductor (add) loading with **full control of load and range** | seated is not the functional plane; "builds glutes" is glute *med*, not max (see Variant selection) | upright → glute med; **for a frontal-plane control deficit this is the *load* tool** — band walks / lateral lunges are the *control* tool; use both, don't swap one for the other | keep; Cavaliere over-downgrades it | keep for *load*; for *control* add Lateral Band Walk, Cossack (ladder), Copenhagen Plank (adductors) |
| **Lateral Raise machine** | strict path | pads fix the forearm → shoulder can't rotate out → impingement-prone at the top | DB or cable with **thumb slightly above pinky** | prefer DB/cable | **DB Lateral Raises** (thumb slightly up) or cable, leaning away |
| **Preacher Curl (flat pad)** | biceps at long length | elbow becomes a rigid fulcrum, distal tendon exposed at heavy loads | choose a pad that slopes away; standing with elbows braced is fine | caution | Biceps Curl standing with elbows braced, or incline DB curl (long head) |
| **Chest Fly machine (pec deck, chest)** | pec adduction | fixed arc; start position can **overstretch the anterior capsule** if arm span doesn't match | set the start inside your range, not at the machine's | caution; cable crossover is the same movement without the fixed arc | **Cable Crossover**; or Incline DB Press |
| **Chest-supported Row** | mid-back with **zero spinal load** — the go-to when the back or neck is irritated | the chest pad tends to round the shoulders forward | chest lifts off the pad slightly, glutes lightly tensed on the prone version | **good** | — (it *is* the alternative to bent-over rows when the spine is irritable); Dual Pulley Row if the pad rounds you |
| **Seated Dip machine** | triceps/lower chest when the shoulder girdle shouldn't take body weight (bar dips off the table) | — | arms travel slightly behind the torso → long head | good alternative to bar dips | — (alternative to bar Dips when the shoulder girdle shouldn't take body weight) |
| **Leg Press** | quad loading with a supported back; controlled ROM — the **return-from-injury** tool | no full hip extension → glutes under-stimulated; heavy loads shift to ligaments; not a squat replacement | feet high → less knee, more hip; treat as a supplement | good, with the caveat | **Hack Squat** (angled platform), **Belt Squat**; Front Squat when the back is fine; pair with **Machine Hip Thrust** for the missing hip extension |
| **Smith machine** | spotter-free pressing | fixed bar path forces the body to fit the machine; squat ankle mechanics distorted; **balance is taken over, so the number lies** | pressing yes; squatting only if nothing else | caution | free bar for squats; **Chest Press / Machine Shoulder Press** for spotter-free pressing |
| **Machine Shoulder Press (converging)** | wrist–elbow–shoulder in one line; **no standing lumbar extension** | — | sit **2 cm off the backrest** so the scapulae can move | **very good** — the overhead choice when the neck or low back is irritable | — (this *is* the alternative to standing OHP when neck or low back is irritable) |
| **Rear-delt Pec Deck** | rear delt / rhomboids **seated** — removes the lumbar lean-back that standing face pulls invite | entering/exiting the handles can jar the shoulder | turn the torso, take one handle, then the other | **very good** | — (alternative to standing Face Pulls when the lumbar leans back) |
| **Chest Press machine** | elbows guided to ~45–60° — shoulder-friendly; safe to take to fatigue alone | — | — | **very good** | — |
| **Leg Extension** | pure quad isolation; the only way to load quads with zero hip or spine involvement | **high patellofemoral pressure at heavy loads** (peak in the 90→45° range) | irritable knee → light load, **terminal range 0–40°**, rehab tempo; reclined seat → rectus femoris, upright → vasti | good, dose it | **Spanish Squat** (isometric, knee-friendly); Leg Press or Hack with feet high; keep extension light and terminal-range |
| **Hack Squat** | angled platform helps ankle dorsiflexion; back supported; good front-squat alternative when wrists/thoracic limit the barbell | still axial load through the shoulders | feet high on the platform → less knee | **good** | — (this *is* the front-squat alternative) |
| **Reverse Hack** | forward lean → glutes | no final hip extension | pair with a full-extension movement (hip thrust) | ok | **Machine Hip Thrust** for the missing full extension |
...
| **Viking / landmine press** | standing press with neutral grip and a **forward-angled path** — less lumbar extension than a strict overhead bar, scapulae free | needs the implement | — | good overhead alternative | Machine Shoulder Press; seated DB Shoulder Press, neutral grip |
| **Seated Cable Row** | free torso — but see the correction | Cavaliere cues torso swing "to open the chest"; **for a lax spine that swing is lumbar flexion/extension under load** | torso still, ribs down; elbows low and tucked → lats, high and wide → upper back | good — **with a still torso** | **Chest-supported Row** if the torso won't stay still |
| **Belt Squat** | **removes spinal compression entirely**; pelvis sits into a good position; full squat pattern | rare in commercial gyms | hands may assist heavy reps | **the squat of choice during any back episode** | if none: **Hack Squat** feet high, Goblet Squat, Leg Press — the no-spine options; Front Squat when the back is fine |
| **Standing Calf Raise** | heavy gastrocnemius loading | the Achilles spring hides the muscle work | **4 s pause at the bottom** removes the stretch-shortening rebound | good | Single-leg Heel Raise (Alfredson) when the goal is tendon rehab, not size |
| **Cable Crossover** | full pec adduction in a safe stretch | — | high→low for lower chest, low→high for upper | good | — |
| **Lat Pulldown** | dosable vertical pull when pull-ups are too much or too few | — | vary grips (see Variant selection) | **very good** | Pull-up / Weighted Pull-up when load is enough |
| **Reverse Hyper** | erector work **through hip extension with the torso fixed** — no axial compression, rhythmic lumbar decompression | evidence is mechanistic and anecdotal (no trials); rare machine | light, rhythmic, high reps | good when available — candidate for an extension-preference back | if none: **45° Back Extension** in the neutral-spine / posterior-tilt variant (glute-biased), Machine Hip Thrust |
| **Hip Thrust machine** | resistance exactly at full hip extension — the one thing squats can't give; easy to set up, no bar on the pelvis | slow eccentrics hold the knee at ~90° under load → patellofemoral cost if the knee is irritable | shin vertical at lockout; ribs down; **normal tempo** | **very good** | Single-leg Glute Bridge, Cable Glute Kickback if the knee objects to the 90° position |
